...
class TinyVersatilePoseGraphSLAM{
    public:
        ...
        static std::pair<Eigen::SparseMatrix<double>, Eigen::SparseMatrix<double>> get_AtPA_AtPB_pose_graph_tait_byan(const std::vector<Eigen::Affine3d> &m_poses, const std::vector<EdgeTaitBryan> &tb_edges);

        static double apply_result_tait_bryan(const Eigen::SparseMatrix<double> &x, std::vector<Eigen::Affine3d> &m_poses);
